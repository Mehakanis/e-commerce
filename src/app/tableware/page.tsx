...
import { tableware, } from "@/sanity/lib/queries";
...
export default async function NewCeramics() {
  const data: Data[] = await sanityFetch({ query: tableware });

  return (
    <main className="max-w-screen-2xl min-h-screen mx-auto">
<Header1/>
    <section className="pb-28  px-20">

      <div className="flex flex-wrap justify-left gap-24 mt-10 text-[#2A254B]">
        ...
        {data.map((product) => (
          <div key={product._id} className="w-[280px] ">
            ...
            <p className=" pb-4 text-[16px]">£{product.price}</p>
            </Link>
          </div>
        ))}
      </div>
